...
class SpatialUnitSpecSortTest {

    @Mock
    private Root<SpatialUnit> root;
    @Mock
    ...
    @Test
    void orderByActionsCount_ascending_setsAscOrderOnQuery() {
        when(root.<List<?>>get("relatedActionUnitList")).thenReturn(collectionPath);
        when(cb.size(collectionPath)).thenReturn(sizeExpr);
        when(cb.asc(sizeExpr)).thenReturn(order);
        when(cb.conjunction()).thenReturn(null);

        Specification<SpatialUnit> spec = SpatialUnitSpec.orderByActionsCount(Sort.Direction.ASC);
    ...
    void orderByRecordingUnitCount_descending_setsDescOrderOnQuery() {
        ...
        Specification<SpatialUnit> spec = SpatialUnitSpec.orderByRecordingUnitCount(Sort.Direction.DESC);
        spec.toPredicate(root, query, cb);

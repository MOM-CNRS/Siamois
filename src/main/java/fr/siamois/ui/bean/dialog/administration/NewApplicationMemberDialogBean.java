 ...
 * search &amp; select existing users and profiles through multi-select autocompletes, optionally
 * invite new users by e-mail, then submit everything at once — newly invited users are created only
 * on final submission, together with the already-existing ones.
 * <p>
...
@Getter
@Setter
public class NewApplicationMemberDialogBean implements Serializable {
    ...
    /** Name displayed in the invitation email when inviting from the application-level user management. */
    private static final String APPLICATION_CONTEXT_NAME = "SIAMOIS";
    ...
    public void exit() {
        reset();
        PrimeFaces.current().executeScript("PF('newApplicationMemberDialog').hide();");
    }
    ...
    private PersonDTO createInvitedPerson(PersonDTO draft) {
        ...
        try {
            ...
            pendingPersonService.invitePersonByEmail(created.getEmail(), APPLICATION_CONTEXT_NAME, langBean.getLanguageCode());
            return created;

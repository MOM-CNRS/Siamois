...
/**
 * Backs the "Ajouter des membres" wizard dialog for a project: search &amp; select existing
...
@Getter
@Setter
public class NewProjectMemberDialogBean implements Serializable {
    ...
    public void exit() {
        reset();
        PrimeFaces.current().executeScript("PF('newProjectMemberDialog').hide();");
    }
     ...
     * existed and the ones just created — to the project via the {@link ProcessPerson} callback.
    ...
    private PersonDTO createInvitedPerson(PersonDTO draft) {
        ...
        try {
            ...
            pendingPersonService.invitePersonByEmail(created.getEmail(), project.getName(), langBean.getLanguageCode());
            return created;

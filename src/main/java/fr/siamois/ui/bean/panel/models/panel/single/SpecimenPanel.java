...
public class SpecimenPanel extends AbstractSingleEntityPanel<SpecimenDTO>  implements Serializable {
    ...
    // Same fix as RecordingUnitPanel/ActionUnitPanel (migration plan, lot 0): without these, an
    // overview pane opened on a "find" row falls the WHOLE (React) project panel back to JSF —
    // AbstractPanel.isReactPanelActive() requires the overview's own isReactPanelEnabled() too.
    @Override
